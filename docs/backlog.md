...
| BUG-006 | bugfix | `ProductCard.jsx`, `ProductDetails.jsx`, `CartView.jsx` desestructuran `product.imagesUrl` (array); el campo real del modelo `Product` es `imageURL` (string, singular). Ningún producto real muestra su imagen — siempre cae al placeholder. Detectado por `anti-hallucination-reviewer` durante la auditoría de `TEST-000`, preexistente, no introducido por ese pendiente | Alta | Pendiente |
